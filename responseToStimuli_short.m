...
while subjectID_ok == 0
    ...
    subject_files = dir([outputPath '/' subjectID '_responseToStimuli*.txt']);
    expected_num_file=beforeAfter-1+sessionNum-1;
    if length(subject_files)~=expected_num_file
        warning_msg = ['There are ' num2str(length(subject_files)) ' files for this subject: ' subjectID '. I expected to find ' num2str(expected_num_file) ' files. Please make sure you entered the right number'];
...
last_fixation_length = 7;
stimLength = 2; % the length of each stimulus presentation
% load the training onsetlist
load(['./Onset_files/train_onset_',num2str(randi([1,4])),'.mat']);
% add to the trainig interval the difference between the 1 sec presentation
% during training, and the desired presentation length.
onsetlist=(stimLength-1)*[0:(length(onsets)-1)]+onsets;
...
fclose(fid1);
WaitSecs(last_fixation_length); % last fixation
...
okHowMany=false;
subjectAnswerHowManyFemaleStimuli=[];
subjectAnswerHowManyMaleStimuli=[];
while ~okHowMany
    % calculate male-female information
    if response_to_male == 1
        howManyMaleStimuli = sum(isMale==1);
        fprintf('There were %d male stimuli in this run\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n',howManyMaleStimuli); % many 'enters' so the subject will not see the answer
        subjectAnswerHowManyMaleStimuli = input('How many male stimuli did the subject count?: ');
    else
        howManyFemaleStimuli = sum(isMale==0);
        fprintf('There were %d female stimuli in this run\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n',howManyFemaleStimuli); % many 'enters' so the subject will not see the answer
        subjectAnswerHowManyFemaleStimuli = input('How many female stimuli did the subject count?: ');
    end
    if ~isempty(subjectAnswerHowManyMaleStimuli)||~isempty(subjectAnswerHowManyFemaleStimuli)
        if isnumeric(subjectAnswerHowManyMaleStimuli)||isnumeric(subjectAnswerHowManyFemaleStimuli)
            okHowMany=true;
        end
    end
end
