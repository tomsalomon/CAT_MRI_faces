...
ret = SMIp_OpenConnection(SMIp, '192.168.2.2', 8888, false); % IP of the host
ret = SMIp_StartRecording(SMIp);


    % make sure that we get gaze data from the Eyelink
    %Eyelink('Command', 'link_sample_data = LEFT,RIGHT,GAZE,HREF,AREA');
    ret = SMIp_SendImageMessage(SMIp, 'Start recording');
...
for trial = 1:trialsPerRun
    ...
    if leftGo(trial) == 1
        ...
        ret = SMIp_SendImageMessage(SMIp,eyelink_message);
    ...
    while noresp
        ...
        if keyIsDown && noresp
            if use_eyetracker
              ...
              ret = SMIp_SendImageMessage(SMIp,['block: ',num2str(block),' run: ',num2str(numRun),' trial: ',num2str(trial),' Press_time: ',num2str(GetSecs)]);
            end
...
%         Screen('DrawText', w, 'You must respond faster!', xcenter-400, ycenter, white);
        CenterText(w,sprintf('You must respond faster!') ,white,0,0);
                ...
                %   Eyelink MSG
                % ---------------------------
                ret = SMIp_SendImageMessage(SMIp,['block: ',num2str(block),' run: ',num2str(numRun),' trial: ',num2str(trial),' Respond_faster_time: ',num2str(GetSecs)]);
                    end
    ...
    fixationTime(trial) = GetSecs - runStart;
                ...
                %   Eyelink MSG
                % ---------------------------
                ret = SMIp_SendImageMessage(SMIp,['block: ',num2str(block),' run: ',num2str(numRun),' trial: ',num2str(trial),' Fixation_ITI_time: ',num2str(GetSecs)]);
...
Postexperiment = GetSecs;
                ...
                %   Eyelink MSG
                % ---------------------------
                ret = SMIp_SendImageMessage(SMIp,['block: ',num2str(block),' run: ',num2str(numRun),' trial: ',num2str(trial),' Part_end_time: ',num2str(GetSecs)]);
                end
...
ret = SMIp_SaveDataRecording( ...
                SMIp, ...
                ['C:\SMI_Data\test_probe',timestamp,'.idf'], ... % Where the data will be saved
                'Descr1', ...
                'User2', ...
                1); % Overwrite - 1 or 0
