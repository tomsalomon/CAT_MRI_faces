...
ROI_GLM_analysis_path_options = dir (sprintf('%s/%s/group_analysis/*%s*anatomical_ROI',group_analysis_path,ses_name,task_name));
...
results_table.std = cellfun(@std,results_table.full_data);
...
data_table_var_names = [ROI_names',strcat(ROI_names','_std'),{'sub','prop','prop_bin'}];
data_empty_mat = nan(numel(Subjects),numel(data_table_var_names));
data_empty_mat(:,num_of_ROI+1:num_of_ROI*2) = 0; % set size=0 for missing ROI
    ...
    for sub_i = 1:numel(Subjects)
        ...
        for ROI_i = 1:num_of_ROI
            ...
            if ~isempty(clsuter_ind)
                data_table(sub_i,data_table_var_names{ROI_i})={results_table.BOLD(clsuter_ind)};
                data_table(sub_i,data_table_var_names{ROI_i+num_of_ROI})={results_table.std(clsuter_ind)};
...
num_interesting_models = sum(cell2mat(model_table_sorted.accuracy) >= 0.6);
